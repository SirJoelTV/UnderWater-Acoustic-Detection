...
WEIGHT_DECAY = 1e-5
MODEL_PATH = "best_gpu_rnn_model.pth"
...
class UnderwaterAcousticDataset(Dataset):
    def __init__(self, data_dir, categories=['ships', 'marine_life'], 
                 sample_rate=SAMPLE_RATE, duration=DURATION, training=True):
    ...
    def _load_dataset(self):
        ...
        for category in self.categories:
            ...
            for subcategory in os.listdir(category_path):
                subcat = os.path.join(category_path, subcategory)
                if not os.path.isdir(subcat):
                    continue
                for filename in os.listdir(subcat):
                    if filename.lower().endswith('.wav'):
                        samples.append({
                            'file': os.path.join(subcat, filename),
...
# ======================== MODEL: GPU-ENABLED BiLSTM ========================
class EnhancedBiLSTM(nn.Module):
    def __init__(self, num_classes, input_size=128, hidden_size=256, num_layers=3):
        super().__init__()
        self.lstm = nn.LSTM(
...
# ======================== MAIN ========================
def main():
    print("\n🚀 Underwater Acoustic Detection (GPU-Enabled)")
    print("="*60)
    ...
    print(f"\n[GPU: {device}] Training on {len(train_loader.dataset)} samples...")
